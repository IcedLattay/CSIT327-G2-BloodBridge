...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function updateBadge() {
    try {
      ...
      notifications.forEach(n => {
        if (!n.is_seen && !shownNotifications.has(n.id)) {
          console.log("New notifications detected! Opening popup overlay...");

          showPopup()                   // Trigger popup once
          shownNotifications.add(n.id);   // Remember that we showed it
        }
      });
  ...
  function renderNotifications(notifications) {
    ...
    notifications.forEach( (n, index) => {
        ...
        item.dataset.requestId = n.request.id;
        ...
        item.innerHTML = `
            ...
            <div class="item-hospital">
                ...
                <p>${n.request.requester.profile.hospital_name}</p>
            </div>
