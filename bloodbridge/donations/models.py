...
class Request(models.Model):
    ...
    def __str__(self):
        compatiblebTypes = ", ".join([bType.type for bType in self.blood_type.compatible_with.all()])
        return f"{self.requester.username} has requested either of the blood types {compatiblebTypes} for blood type {self.blood_type.type} on ({self.date})"